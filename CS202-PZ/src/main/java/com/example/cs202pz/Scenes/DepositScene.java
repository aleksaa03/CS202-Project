...
import com.example.cs202pz.Exceptions.DepositException;
...
/**
 * Represents the scene for depositing money in the ATM application, extending JavaFX Scene and implementing the Resettable interface.
 * Users can input the deposit amount, clear the input, and complete the deposit transaction.
 */
public class DepositScene extends Scene implements Resettable {
    /**
     * The singleton instance of the DepositScene class.
     */
    public static final DepositScene INSTANCE = new DepositScene();
    private static String amount = "";
    private final TextField textField;

    /**
     * Constructs the DepositScene, initializing UI elements and setting up event handlers.
     */
    public DepositScene() {
    ...
    /**
     * Adds the given character to the deposit amount field after validation.
     *
     * @param amountChar The character to be added to the deposit amount.
     */
    ...
    /**
     * Removes the last character from the deposit amount field.
     * Does nothing if the deposit amount is already empty.
     */
    ...
    /**
     * Sets the deposit amount in the text field.
     *
     * @param amount The deposit amount to be displayed.
     */
    ...
    /**
     * Retrieves and returns a button for the deposit action with appropriate event handling.
     *
     * @return The Button for the deposit action.
     */
    private Button getButton() {
        final Button button = new Button(Constants.deposit);
        button.setOnAction(e -> {
            try {
                final double amount = Double.parseDouble(textField.getText());
                Client.getHandler().send(new DepositRequest(DbManager.getAccountID(), amount, false));

                DepositRequest response = (DepositRequest) Client.getHandler().tryReceive();
                if (response.getStatus() == TransactionStatus.DECLINED) {
                    throw new DepositException(Constants.depositDeclined);
                }

                Application.Alert(Alert.AlertType.INFORMATION, Constants.deposit, Constants.depositSuccessful);
            } catch (Exception ex) {
                try {
    ...
    /**
     * Validates whether the given character can be added to the deposit amount field.
     *
    ...
    /**
     * Clears the deposit amount, resetting it to an empty state.
     */
    private void clear() {
        amount = "";
        setAmount(amount);
    ...
    /**
     * Resets the DepositScene by clearing the accumulated deposit amount.
